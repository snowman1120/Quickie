...
Future<Stream<Order>> getOrders() async {
  ...
  final String url =
      '${GlobalConfiguration().getValue('api_base_url')}orders?${_apiToken}with=user;productOrders;productOrders.product;orderStatus;deliveryAddress;payment&search=user.id:${_user.id}&searchFields=user.id:=&orderBy=id&sortedBy=desc';
  print(url);
...
Future<Stream<Order>> getRecentOrders() async {
  ...
  final String _apiToken = 'api_token=${_user.apiToken}&';
  final String url =
      '${GlobalConfiguration().getValue('api_base_url')}orders?${_apiToken}with=user;productOrders;productOrders.product;orderStatus;deliveryAddress;payment&search=user.id:${_user.id}&searchFields=user.id:=&orderBy=updated_at&sortedBy=desc&limit=3';
...
Future<Order> addOrder(Order order, Payment payment) async {
  ...
  print(_creditCard.number);
  print(_creditCard.cvc);
  print(_creditCard.expYear);
  order.user = _user;
