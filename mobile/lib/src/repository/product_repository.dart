...
Future<Stream<Product>> getProduct(String productId) async {
  ...
  uri = uri.replace(queryParameters: {
    'with':
        'store;category;brand;options;optionGroups;productReviews;productReviews.user',
...
Future<Stream<Product>> searchProducts(String search) async {
  ...
  uri = uri.replace(queryParameters: {
    // 'search':
    //     'name:$search;description:$search;category_name:$search;itemsAvailable:0',
...
Future<Stream<Product>> getProductsOfStore(String marketId) async {
  final String url =
      '${GlobalConfiguration().getValue('api_base_url')}products?with=store&search=store.id:$marketId&searchFields=store.id:=';
  try {
    final client = new http.Client();
    final streamedRest = await client.send(http.Request('get', Uri.parse(url)));
...
Future<Stream<Review>> getProductReviews(String id) async {
  final String url =
      '${GlobalConfiguration().getValue('api_base_url')}product_reviews?with=user&search=product_id:$id';
  try {
    final client = new http.Client();
    final streamedRest = await client.send(http.Request('get', Uri.parse(url)));
