...
Future<Stream<Store>> searchStores(String search, Address address) async {
  ...
  final String url =
      '${GlobalConfiguration().getValue('api_base_url')}stores?$_searchParam&$_orderLimitParam';
  try {
    final client = new http.Client();
...
Future<Stream<Review>> getStoreReviews(String id) async {
  final String url =
      '${GlobalConfiguration().getValue('api_base_url')}store_reviews?with=user&search=store_id:$id';
  try {
    final client = new http.Client();
    final streamedRest = await client.send(http.Request('get', Uri.parse(url)));
...
Future<Stream<Review>> getRecentReviews() async {
  final String url =
      '${GlobalConfiguration().getValue('api_base_url')}store_reviews?orderBy=updated_at&sortedBy=desc&limit=3&with=user';
  try {
    final client = new http.Client();
    final streamedRest = await client.send(http.Request('get', Uri.parse(url)));
    return streamedRest.stream
...
Future<Review> addStoreReview(Review review, Store store) async {
  final String url =
      '${GlobalConfiguration().getValue('api_base_url')}store_reviews';
  final client = new http.Client();
