...
class ProductController extends ControllerMVC {
  ...
  final _productsByCategoriesStreamController =
      StreamController<List<Product>>();

  void disposeStreams() {
    _productsByCategoriesStreamController.close();
  }

  Function(List<Product>) get productByCategorieSink =>
      _productsByCategoriesStreamController.sink.add;

  Stream<List<Product>> get productByCategorieStream =>
      _productsByCategoriesStreamController.stream;
  ...
  void listenForProductsByCategory({String id, String message}) async {
    ...
    stream.listen((Product _product) {
      setState(() {
        ...
        productByCategorieSink(categoriesProducts);
      });
    }, onError: (a) {
      print(a);
    });
        ...
        if (product.options != null) {
          ...
          if (_oldCart != null) {
            if (_oldCart.quantity + this.quantity <=
                double.parse(product.itemsAvailable)) {
  ...
  void addToFavorite(Product product, BuildContext context) async {
  ...
  void removeFromFavorite(Favorite _favorite, BuildContext context) async {
    ...
    removeFavorite(_favorite).then((value) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(
              S.of(state.context).the_product_was_removed_from_your_favorite)));
    });
  }
